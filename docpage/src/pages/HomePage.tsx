...
const quickLinks = [
  { to: '/quickstart',   icon: '▶', title: '快速开始',    desc: '5 分钟部署运行' },
  { to: '/architecture', icon: '⬡', title: '系统架构',    desc: '了解设计原理' },
  { to: '/api',          icon: '⚡', title: 'API 文档',    desc: 'REST API 参考' },
  { to: '/driver',       icon: '⚙', title: 'PKCS#11 驱动', desc: '驱动集成指南' },
]
      ...
      <section className="hero">
        <div className="hero-content">
          ...
          <div className="hero-actions animate-fade-in-up delay-4">
            <Link to="/quickstart" className="btn-primary">快速开始 →</Link>
            <Link to="/overview" className="btn-secondary">功能概览</Link>
            <a
      ...
      <section className="section">
        <div className="section-inner">
          <div className="section-header animate-fade-in-up">
            ...
            <p className="section-desc">三层架构设计，驱动层、客户端、云端服务各司其职</p>
          </div>
          <div className="arch-flow animate-fade-in-up delay-1">
      ...
      <section className="section section-alt">
        <div className="section-inner">
          <div className="section-header animate-fade-in-up">
            ...
            <p className="section-desc">覆盖证书全生命周期管理的完整功能集</p>
          </div>
          ...
          <div className="nav-cards animate-fade-in-up delay-1">
            {quickLinks.map((link) => (
              <Link key={link.to} to={link.to} className="nav-card">
                <span className="nav-card-icon">{link.icon}</span>
                <div>
                  <div className="nav-card-title">{link.title}</div>
                  <div className="nav-card-desc">{link.desc}</div>
                </div>
              </Link>
            ))}
          </div>
