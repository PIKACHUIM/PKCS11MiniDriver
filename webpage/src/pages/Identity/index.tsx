...
import {
  ...
  Tag, Typography, message, Popconfirm, Alert, Descriptions,
} from 'antd';
...
const SubjectInfoTab: React.FC = () => {
  ...
  const load = async (p = 1) => {
    setLoading(true);
    try {
      const res = await listSubjectInfos({ page: p, page_size: 20 });
      setData(res.items || []);
      setTotal(res.total);
    ...
    <>
      ...
      <Modal title="添加主体信息" open={open} onCancel={() => { setOpen(false); form.resetFields(); setSelectedTemplate(null); }} footer={null} width={520}>
        <Form form={form} layout="vertical" onFinish={handleCreate} style={{ marginTop: 16 }}>
          <Form.Item label="选择主体模板" name="template_uuid" rules={[{ required: true, message: '请选择模板' }]}>
            <Select placeholder="请选择主体模板" onChange={(uuid) => {
              const t = templates.find(t => t.uuid === uuid);
...
const ExtensionInfoTab: React.FC = () => {
  ...
  const load = async (p = 1) => {
    setLoading(true);
    try {
      const res = await listExtensionInfos({ page: p, page_size: 20 });
      setData(res.items || []);
      setTotal(res.total);
    } catch (e: any) { message.error(e.message); }
    finally { setLoading(false); }
  ...
  const handleCreate = async (values: { type: string; value: string }) => {
    setSaving(true);
    try {
      await createExtensionInfo(values);
      message.success('扩展信息已添加，请完成验证');
      setOpen(false); form.resetFields(); load();
    } catch (e: any) { message.error(e.message); }
    finally { setSaving(false); }
  };

  const handleVerifyDNS = async (record: ExtensionInfo) => {
    try {
      await verifyDNS(record.uuid);
      message.success('DNS 验证成功');
      load();
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      await verifyEmail(verifyTarget.uuid, emailCode);
      message.success('邮箱验证成功');
      setVerifyEmailOpen(false); setEmailCode(''); load();
      ...
      title: '操作', key: 'action', render: (_: any, r: ExtensionInfo) => (
        <Space size={4}>
          {r.status === 'pending' && r.verify_method === 'dns' && (
            <Button size="small" type="primary" onClick={() => handleVerifyDNS(r)}>验证 DNS</Button>
          )}
  ...
  return (
    <>
        ...
        expandable={{
          expandedRowRender: (r: ExtensionInfo) => r.verify_method === 'dns' && r.status === 'pending' ? (
            <Alert type="info" message="DNS 验证说明"
              description={<>请在您的 DNS 服务商处添加以下 TXT 记录：<br /><Text code>_opencert.{r.value} TXT opencert-verify={r.verify_token || '（请刷新获取 token）'}</Text><br />添加后点击「验证 DNS」按钮。</>}
            />
          ) : null,
        }}
        pagination={{ current: page, total, pageSize: 20, onChange: (p) => { setPage(p); load(p); } }}
      ...
      <Modal title="添加扩展信息" open={open} onCancel={() => { setOpen(false); form.resetFields(); }} footer={null}>
        <Form form={form} layout="vertical" onFinish={handleCreate} style={{ marginTop: 16 }}>
          ...
          <Form.Item label="值" name="value" rules={[{ required: true, message: '请输入值' }]}>
            <Input placeholder="example.com / user@example.com / 192.168.1.1" />
          </Form.Item>
          <Form.Item style={{ marginBottom: 0, textAlign: 'right' }}>
      ...
      <Modal title="邮箱验证" open={verifyEmailOpen} onCancel={() => setVerifyEmailOpen(false)}
        onOk={handleVerifyEmail} okText="验证">
