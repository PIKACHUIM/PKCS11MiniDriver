...
import {
  ...
  Tag, Typography, Drawer, message, Popconfirm, Tooltip,
} from 'antd';
import {
  PlusOutlined, DownloadOutlined, ImportOutlined,
  UnorderedListOutlined, SafetyCertificateOutlined, DeleteOutlined,
} from '@ant-design/icons';
import {
  listCAs, createCA, deleteCA, importCAChain,
  listRevokedCerts, revokeCAcert, issueCert, downloadCRL,
} from '../../api';
...
const CA_Page: React.FC = () => {
    ...
    { title: '已签发', dataIndex: 'issued_count', key: 'issued_count' },
    {
      ...
      render: (_: any, r: CA) => (
        <Space size={4} wrap>
          ...
          <Popconfirm title="确认删除此 CA？" onConfirm={() => handleDelete(r.uuid)}>
  ...
  return (
    <div>
      ...
      <Table
        rowKey="uuid" columns={columns} dataSource={cas} loading={loading}
      ...
      <Modal title="创建 CA" open={createOpen} onCancel={() => setCreateOpen(false)} footer={null} width={520}>
        <Form form={createForm} layout="vertical" onFinish={handleCreate} style={{ marginTop: 16 }}>
          ...
          <Form.Item label="密钥类型" name="key_type" rules={[{ required: true }]}>
            <Select>
              {['ec256', 'ec384', 'rsa2048', 'rsa4096'].map(k => <Option key={k} value={k}>{k}</Option>)}
            </Select>
          </Form.Item>
      ...
      <Modal title={`导入证书链 — ${selectedCA?.name}`} open={chainOpen} onCancel={() => setChainOpen(false)} footer={null} width={560}>
        <Form form={chainForm} layout="vertical" onFinish={handleImportChain} style={{ marginTop: 16 }}>
          ...
          <Form.Item style={{ marginBottom: 0, textAlign: 'right' }}>
            <Space>
              <Button onClick={() => setChainOpen(false)}>取消</Button>
              <Button type="primary" htmlType="submit" loading={chainLoading}>导入</Button>
            </Space>
          </Form.Item>
        </Form>
      </Modal>
        ...
        <Form form={issueForm} layout="vertical" onFinish={handleIssue} style={{ marginTop: 16 }}>
          ...
          <Form.Item style={{ marginBottom: 0, textAlign: 'right' }}>
            <Space>
              ...
              <Button type="primary" htmlType="submit" loading={issueLoading}>签发</Button>
            </Space>
          </Form.Item>
        </Form>
      </Modal>
      ...
      {/* 吊销列表 Drawer */}
      <Drawer
        title={`吊销列表 — ${selectedCA?.name}`}
        open={revokedDrawer} onClose={() => setRevokedDrawer(false)} width={600}
        extra={<Button type="primary" danger onClick={() => setRevokeOpen(true)}>吊销证书</Button>}
      >
        <Table
          rowKey="serial" size="small"
          dataSource={revokedList}
          columns={[
        ...
        <Form form={revokeForm} layout="vertical" onFinish={handleRevoke} style={{ marginTop: 16 }}>
          ...
          <Form.Item style={{ marginBottom: 0, textAlign: 'right' }}>
            <Space>
              ...
              <Button type="primary" danger htmlType="submit" loading={revokeLoading}>确认吊销</Button>
            </Space>
          </Form.Item>
        </Form>
      </Modal>
