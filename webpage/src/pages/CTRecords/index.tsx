...
const CTRecords: React.FC = () => {
  ...
  const load = async (p = 1) => {
    setLoading(true);
    try {
      const res = await listCTEntries({
        cert_hash: certHash || undefined,
        cert_uuid: certUUID || undefined,
        page: p,
        page_size: 20,
      });
      setData(res.items || []);
  ...
  const handleDelete = async (uuid: string) => {
    try {
      await deleteCTEntry(uuid);
      message.success('CT 记录已删除');
      load(page);
    } catch (e: any) {
  ...
  return (
    <div>
      ...
      <Space style={{ marginBottom: 16 }} wrap>
        ...
        <Button onClick={() => { setCertHash(''); setCertUUID(''); setTimeout(() => load(1), 0); }}>
          重置
        </Button>
      </Space>
