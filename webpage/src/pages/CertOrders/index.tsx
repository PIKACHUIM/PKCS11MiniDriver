...
import {
  ...
  Tag, Typography, message, Card, Row, Col, Descriptions,
} from 'antd';
          ...
          {templates.map(t => (
            <Col xs={24} sm={12} lg={8} key={t.uuid}>
              <Card hoverable style={{ height: '100%' }}
                actions={[
                  <Button type="primary" icon={<ShoppingCartOutlined />} onClick={() => { setSelected(t); setBuyOpen(true); }}>
                    立即购买
                  </Button>
                ]}
              >
                <Tag color={{ ssl: 'blue', code_sign: 'purple', email: 'green', custom: 'orange' }[t.category] || 'default'} style={{ marginBottom: 8 }}>
                  {t.category}
                </Tag>
                <Title level={5} style={{ margin: '0 0 8px' }}>{t.name}</Title>
                <Text type="secondary" style={{ fontSize: 13 }}>
                  价格：{t.price === 0 ? '免费' : `¥${(t.price / 100).toFixed(2)}`}
                </Text>
                <br />
                <Text type="secondary" style={{ fontSize: 13 }}>
                  有效期：{t.validity_options?.join(' / ')} 天
                </Text>
                <br />
                <Text type="secondary" style={{ fontSize: 13 }}>
                  密钥类型：{t.allowed_key_types?.join(', ')}
                </Text>
              </Card>
            </Col>
          ))}
      ...
      <Modal title={`购买证书 — ${selected?.name}`} open={buyOpen} onCancel={() => setBuyOpen(false)} footer={null}>
        <Form form={form} layout="vertical" onFinish={handleBuy} style={{ marginTop: 16 }}>
          <Form.Item label="有效期" name="validity_days" rules={[{ required: true }]}>
            <Select>
              {(selected?.validity_options || []).map(d => <Option key={d} value={d}>{d} 天</Option>)}
            </Select>
          </Form.Item>
          <Form.Item label="密钥类型" name="key_type" rules={[{ required: true }]}>
            <Select>
              {(selected?.allowed_key_types || KEY_TYPES).map(k => <Option key={k} value={k}>{k}</Option>)}
            </Select>
          </Form.Item>
...
const OrdersTab: React.FC = () => {
  ...
  const load = async (p = 1) => {
    setLoading(true);
    try {
      const res = await listCertOrders({ page: p, page_size: 20 });
  ...
  const handleApply = async (values: any) => {
    ...
    try {
      await createCertApplication({
        ...
        subject_info_uuid: values.subject_info_uuid,
        extension_info_uuids: values.extension_info_uuids || [],
        ...
        <Form form={form} layout="vertical" onFinish={handleApply} style={{ marginTop: 16 }}>
            ...
            <Select placeholder="请选择主体信息">
              {subjectInfos.map(s => (
                <Option key={s.uuid} value={s.uuid}>
                  {s.template_name} — {Object.values(s.fields || {}).slice(0, 2).join(', ')}
                </Option>
              ))}
            </Select>
          </Form.Item>
          <Form.Item label="选择扩展信息（已验证，可多选）" name="extension_info_uuids">
            <Select mode="multiple" placeholder="请选择已验证的域名/邮箱/IP">
              {extensionInfos.map(e => (
                <Option key={e.uuid} value={e.uuid}>{e.type}: {e.value}</Option>
              ))}
            </Select>
          </Form.Item>
          <Form.Item label="密钥类型" name="key_type" rules={[{ required: true }]}>
...
const ApplicationsTab: React.FC = () => {
  ...
  const load = async (p = 1) => {
    setLoading(true);
    try {
      const res = await listCertApplications({ page: p, page_size: 20 });
    ...
    <>
      ...
      <Modal title="拒绝申请" open={rejectOpen} onCancel={() => setRejectOpen(false)}
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <Select.Option value="" />
          <input
            style={{ width: '100%', marginTop: 8, padding: '4px 8px', border: '1px solid #d9d9d9', borderRadius: 6 }}
            placeholder="请输入拒绝原因"
            value={rejectReason}
