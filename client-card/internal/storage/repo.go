...
func (r *UserRepo) Create(ctx context.Context, u *User) error {
	...
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO users (uuid, user_type, display_name, email, enabled, cloud_url, password_hash, auth_token, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		u.UUID, string(u.UserType), u.DisplayName, u.Email,
...
func (r *UserRepo) GetByUUID(ctx context.Context, userUUID string) (*User, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		SELECT uuid, user_type, display_name, email, enabled, cloud_url, password_hash, auth_token, created_at, updated_at
		FROM users WHERE uuid = ?`, userUUID).
		Scan(&u.UUID, &u.UserType, &u.DisplayName, &u.Email,
...
func (r *UserRepo) List(ctx context.Context) ([]*User, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT uuid, user_type, display_name, email, enabled, cloud_url, created_at, updated_at
	...
	for rows.Next() {
		u := &User{}
		var enabled int
		if err := rows.Scan(&u.UUID, &u.UserType, &u.DisplayName, &u.Email,
...
func (r *UserRepo) Update(ctx context.Context, u *User) error {
	...
	_, err := r.db.ExecContext(ctx, `
		UPDATE users SET user_type=?, display_name=?, email=?, enabled=?, cloud_url=?, password_hash=?, auth_token=?, updated_at=?
		WHERE uuid=?`,
		string(u.UserType), u.DisplayName, u.Email, boolToInt(u.Enabled),
